...
public class DropInfostringItemAction implements ChatAction {
	...
	public void fire(final Player player, final Sentence sentence, final EventRaiser npc) {
		...
		boolean res = false;
		int numberOfItems = 0;
		int itemsWithInfostring = 0;
		int itemsWithoutInfostring = 0;
		for (final Item item : items) {
			numberOfItems++;
			if (infostring.equalsIgnoreCase(item.getInfoString()))
				itemsWithInfostring++;
			else
				itemsWithoutInfostring++;
		}
		res = player.drop(itemName, numberOfItems);
		Item theItem;
		for (int i = 1; i <= itemsWithoutInfostring; i++) {
			theItem = SingletonRepository.getEntityManager().getItem(itemName);
			player.equipOrPutOnGround(theItem);		
		}
		for (int i = 1; i < itemsWithInfostring; i++) {
			theItem = SingletonRepository.getEntityManager().getItem(itemName);
			theItem.setInfoString(infostring);
			player.equipOrPutOnGround(theItem);
		}
