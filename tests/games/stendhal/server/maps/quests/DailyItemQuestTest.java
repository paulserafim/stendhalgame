...
public class DailyItemQuestTest {
	...
	public void testQuest() {
		...
		//-------------------------------------------------
		//Change the quest of the player for a great number of week so it's impossible
		//to have two same quests in row
		String previousItem = null;
		for (int weekNumber = 1; weekNumber < 3000; weekNumber++) {
			player.setQuest(questSlot, 1, "0");
			en.step(player, "hi");
			en.step(player, "task");
			en.step(player, "abort");
			en.step(player, "bye");
			
			String currentItem = player.getQuest(questSlot, 0);
			assertNotEquals("Error on week: " + weekNumber, currentItem, previousItem);
